...
void testBtree1(char **fileLocation)
{
    printf("\nStart running Btree 1 tests\n");
    Btree *pBt;
    char zBuf[100];
    int rc;
    rc = sqliteBtreeOpen(*fileLocation, 0666, 10, &pBt);
    ...
    {
        char *keyToWrote2 = "key20345094830594830580398485739470583049504857237487329580438059840593875987239473850943850997593287493794879285794835394805980239" + i;
        char key[strlen(keyToWrote2) + 4];
        sprintf(key, "%s%d", keyToWrote2, i);

        char *valToWrite2 = "value2a4444499000233234234324234234384023984092830498203948283749623849938423804598059834-059340-5934-0959-20394-203768723487248" + i;
        char value[strlen(valToWrite2) + 4];
        sprintf(value, "%s%d", valToWrite2, i);

        sqliteBtreeBeginTrans(pBt);
        sqliteBtreeInsert(pCur, key, strlen(key), value, strlen(value)); // Insert value to Btree
        sqliteBtreeCommit(pBt);
    }
    sqliteOsDelete(*fileLocation);
}
